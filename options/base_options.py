...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--dataroot', default='./dataset/ForenSynths', help='dataset root containing train/val/test splits')
    ...
    def parse(self, print_options=True):
        ...
        if len(opt.gpu_ids) > 0:
            if not torch.cuda.is_available():
                raise RuntimeError(
                    "GPU was requested via --gpu_ids, but CUDA is not available in the current Python environment. "
                    "Use the CUDA-enabled virtualenv interpreter or run with --gpu_ids -1 for CPU."
                )
